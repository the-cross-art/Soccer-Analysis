...
exports.upload = async (req, res) => {
  let result = await cloudinary.v2.uploader.upload(req.body.image, {
    folder: `Soccer/${req.user.name}id:${req.user.user_id}`,
...
exports.uploadImage = async (req, res) => {
  const { image, folder, subfolder } = req.body;
  try {
    let result = await cloudinary.v2.uploader.upload(image, {
      folder: `Soccer/${req.user.name + req.user.user_id}/${folder}/${(subfolder?subfolder:'')}`,
      public_id: `${Date.now()}`,
      resource_type: "auto", //jpeg, png
    });
