...
exports.create = async (req, res) => {
  ...
  try {
    const user = await User.findById(u_id).exec();
    if (user) {
      ...
      res.status(404).send("Sorry no user found, match can't be created");
  } catch (error) {
    console.log(error);
    res.status(400).json({
      err: error.message,
...
exports.read = async (req, res) => {
  const { id } = req.params;
  try {
    const foundMatch = await Match.findById(id).populate('players').exec();
    if (foundMatch) {
      res.json(foundMatch);
    } else {
    ...
    res.status(404).send("Error in fetching the match");
  }
};

exports.listAll = async (req, res) => {
  const { u_id, count } = req.params;
